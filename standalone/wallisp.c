// wallisp — tiny Lisp compiled to freestanding wasm32 (zero imports).
//
// Architecture: one-pass bytecode compiler over a flat u32 instruction stream,
// stack-machine VM with tail-call optimization, mark-sweep GC over a uniform
// cons-cell arena. Globals are an O(1) array indexed by symbol id.
//
// The collector is non-moving — indices never change, so roots are MARKED,
// never rewritten — and the free list has no fragmentation because every heap
// object is a uniform Cell. GC is disabled during compile (the AST isn't
// rooted) and enabled when run() starts.
//
// Host ABI: input_ptr(), eval_source(len), output_ptr(), gc_count(). Write
// source bytes at input_ptr(); eval_source(len) compiles + runs and returns
// the length of the printed result at output_ptr().
//
// Origin: extracted from engines/bytecode_gc.c in the wallisp study repo —
// see ENGINES.md / FINDINGS.md there for the measurement record that picked
// this engine and these design choices.
...
// ---- tag scheme ------------------------------------------------------------
// Low 2 bits = tag: 00 fixnum (30-bit signed, wraps at ±536M), 01 cons (index
// into the cell arena), 10 symbol (interned, stable), 11 special (nil/t/error
// plus the small range of primitives).
#define TAG_MASK 3u
#define TAG_FIX 0u
#define TAG_CONS 1u
#define TAG_SYM 2u
...
// Slow path (arena full): reuse a freed cell, or collect, or OOM. Out of line
// so the fast path stays free of any call and inlines into allocation sites.
...
// ---- primitives ------------------------------------------------------------
// Arity, type, divide-by-zero, and arithmetic overflow are all errors —
// `(+ 1)`, `(+ 'a 1)`, `(/ 1 0)`, and overflow past 30-bit fixnum range all
// return ERR rather than silently degrading.
...
// Slow path. The hot path (binary +/-/*/=/<, unary car/cdr/null?/pair?/list?)
// is inlined and validated at the VM level — see OP_CALL/OP_TAILCALL below.
// apply_prim handles variadic +/-/* with n≥3 and anything called with a
// non-fast-path arity.
  ...
  for(;;){
    ...
    switch(opc){
      ...
      case OP_SETG: {
        ...
        // on the stack (set! returns the assigned value).
        u32 s=code[ip++];
      ...
      case OP_CALL: {
        ...
        if(is_prim(fn)){
          ...
          if(n==2 && id>=PR_ADD && id<=PR_LT){      // inline 2-arg arith/cmp: no cons
            // PR_EQ stays polymorphic — it compares tagged values directly.
