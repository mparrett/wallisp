...
enum {
  ...
  OP_JMP, OP_JFALSE, OP_CLOSURE, OP_CALL, OP_RET, OP_HALT
};
  ...
  for(;;){
    ...
    switch(opc){
      ...
      case OP_CALL: {
        ...
        if(is_prim(fn)){
          u32 id=fn>>2;
          if(n==2 && id>=PR_ADD && id<=PR_LT){          // inline 2-arg arith/compare: NO cons
            u32 a=vstack[vsp-2], b=vstack[vsp-1], r;
            switch(id){
              case PR_ADD: r=a+b; break;                // tagged fixnums add directly
