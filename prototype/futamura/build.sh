...
"$BUILD/specialize" < "$HERE/closures_v2_demo.lisp"    > "$BUILD/residual_closures_v2_demo_gen.c"
"$BUILD/specialize" < "$HERE/closures_named_demo.lisp" > "$BUILD/residual_closures_named_demo_gen.c"
"$BUILD/specialize" < "$HERE/nrev_demo.lisp"           > "$BUILD/residual_nrev_demo_gen.c"
...
clang "${WASMFLAGS[@]}" -o "$REPO_ROOT/residual_closures_v2_demo_gen.wasm"    "$BUILD/residual_closures_v2_demo_gen.c"
clang "${WASMFLAGS[@]}" -o "$REPO_ROOT/residual_closures_named_demo_gen.wasm" "$BUILD/residual_closures_named_demo_gen.c"
clang "${WASMFLAGS[@]}" -o "$REPO_ROOT/residual_nrev_demo_gen.wasm"           "$BUILD/residual_nrev_demo_gen.c"
...
echo "  $REPO_ROOT/residual_nrev_demo_gen.wasm"
