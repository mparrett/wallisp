...
enum {
  ...
  OP_JMP, OP_JFALSE, OP_CLOSURE, OP_CALL, OP_RET, OP_HALT, OP_PADD, OP_PSUB, OP_PMUL, OP_PEQ, OP_PLT
};
...
static void compile(u32 x,u32 cenv){
  if(is_fix(x) || tagof(x)==TAG_SPEC){ emit(OP_CONST); emit(x); return; }
  if(is_sym(x)){
    u32 d,i;
  ...
  u32 op=car(x);
  if(op==s_quote){ emit(OP_CONST); emit(car(cdr(x))); return; }
  if(op==s_if){
    compile(car(cdr(x)),cenv);
    emit(OP_JFALSE); u32 p1=cp; emit(0);
    compile(car(cdr(cdr(x))),cenv);
    emit(OP_JMP); u32 p2=cp; emit(0);
    code[p1]=cp;
    compile(car(cdr(cdr(cdr(x)))),cenv);
    code[p2]=cp;
    return;
  }
  if(op==s_define){ compile(car(cdr(cdr(x))),cenv); emit(OP_DEFG); emit(symidx(car(cdr(x)))); return; }
  if(op==s_lambda){
    u32 params=car(cdr(x)), body=car(cdr(cdr(x)));
    u32 nparams=0; for(u32 p=params;is_cons(p);p=cdr(p)) nparams++;
    emit(OP_JMP); u32 pj=cp; emit(0);
    u32 body_addr=cp;
    compile(body, cons(params,cenv));
    emit(OP_RET);
    code[pj]=cp;
  ...
  if(op==s_begin){
    u32 b=cdr(x);
    if(!is_cons(b)){ emit(OP_CONST); emit(NIL); return; }
    while(is_cons(cdr(b))){ compile(car(b),cenv); emit(OP_POP); b=cdr(b); }
    compile(car(b),cenv);
    return;
  }
  if(op==s_let){
    ...
    for(u32 bnd=binds;is_cons(bnd);bnd=cdr(bnd)){ vars=cons(car(car(bnd)),vars); vals=cons(car(cdr(car(bnd))),vals); }
    u32 lam=cons(s_lambda,cons(vars,cons(body,NIL)));
    compile(cons(lam,vals),cenv);
    return;
  }
  // superinstruction: (P a b) with P a global symbol still bound to + - * = < -> one opcode
  if(is_sym(op)){
    u32 rd,ri;
    if(!resolve(op,cenv,&rd,&ri)){                 // global, not a shadowing local
      u32 g=gval[symidx(op)];
      if(is_prim(g) && (g>>2)>=PR_ADD && (g>>2)<=PR_LT){
        u32 na=0; for(u32 a=cdr(x);is_cons(a);a=cdr(a)) na++;
        if(na==2){
          for(u32 a=cdr(x);is_cons(a);a=cdr(a)) compile(car(a),cenv);
          emit(OP_PADD + ((g>>2)-PR_ADD));
          return;
        }
      }
    }
  ...
  compile(op,cenv);
  u32 n=0; for(u32 a=cdr(x);is_cons(a);a=cdr(a)){ compile(car(a),cenv); n++; }
  emit(OP_CALL); emit(n);
}
...
#define CALL_MAX   2000000
static u32 vstack[VSTACK_MAX];
static u32 ret_ip[CALL_MAX];
static u32 ret_env[CALL_MAX];

static unsigned g_icount=0;
__attribute__((noinline)) static u32 run(u32 entry){
  ...
  for(;;){
    ...
    switch(opc){
      ...
      case OP_PADD: { u32 b=vstack[--vsp]; vstack[vsp-1]=vstack[vsp-1]+b; break; }
      case OP_PSUB: { u32 b=vstack[--vsp]; vstack[vsp-1]=vstack[vsp-1]-b; break; }
      case OP_PMUL: { u32 b=vstack[--vsp]; vstack[vsp-1]=mkfix(fixval(vstack[vsp-1])*fixval(b)); break; }
      case OP_PEQ:  { u32 b=vstack[--vsp]; vstack[vsp-1]=(vstack[vsp-1]==b)?TRUE:NIL; break; }
      case OP_PLT:  { u32 b=vstack[--vsp]; vstack[vsp-1]=((i32)vstack[vsp-1]<(i32)b)?TRUE:NIL; break; }
...
#define INCAP 8192
char inbuf[INCAP];
__attribute__((export_name("icount"))) unsigned icount(){ return g_icount; }
